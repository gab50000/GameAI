...
instance GameState Checkers where
  ...
  getScore state move = getScoreUpToDepth 5 state playerColor move
   where
...
getScoreUpToDepth n state playerColor move
  ...
  | n > 0
    , Just nextBoard <- maybeNextBoard =
    let moves = getAllMoves nextBoard nextPlayer nextDirection
     in case moves of
          (mv : _) -> - (maximum $ map (getScoreUpToDepth (n -1) (Checkers nextBoard nextPlayer nextDirection) playerColor) moves)
          [] -> 0
 where
...
exampleBoard =
  fromList $
    map
      fromList
      ( List.replicate 3 (List.replicate 3 (Just (Piece White Man)) ++ [Just (Piece White King)])
          ++ List.replicate 2 (List.replicate 4 Nothing)
          ++ List.replicate 3 (List.replicate 3 (Just (Piece Black Man)) ++ [Just (Piece Black King)])
      )
